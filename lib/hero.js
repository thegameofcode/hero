...
function mq(p_config){
	...
	function _notify(p_data) {
		_route(_config.exchange, p_data);
	}
...
if (config.qlog) {
	var options = {
		url  : config.qlog.url.replace(':appkey:', config.qlog.clientId)
	,	json : { p : config.qlog.secretKey}
	};

	console.log('Requesting Qlog Service', options);
	request.post(options, function(err, response, data){
		if ( !err ){
			console.log("Qlog Response data", data)
			var qlog = new mq(extend(config.mq.log, data));
			hero.error = function(message) {
				error(message);
				qlog.notify({
					msg: message,
					tags: tags,
